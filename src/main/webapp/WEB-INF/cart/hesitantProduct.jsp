...
<h1 class="tableName titleName2">
    猶豫商品
    <br>
    <span class="littleName">Hesitant products</span>
</h1>
<div class="foodArea">
    ...
    $.ajax({
        ...
        success: function(carts){
            ...
            for(let i=0; i<carts.length; i++){
            	cartHtml +='<tr><th scope="row">'+(i+1)+'</th>';
            	cartHtml += '<td>'+cartList[i].productId+'</td>';
                let nameObj = new String(cartList[i].product.productName);
                let nameString = nameObj.substring(0,15);
                cartHtml += '<td>'+nameString+'</td>';
                cartHtml += '<td class="total">'+cartList[i].product.productPrice+'</td>';
                cartHtml += '<td class="total">'+cartList[i].product.productSalesFigures+'</td>'; 
                cartHtml += '<td class="total">'+cartList[i].quantity+'</td></tr>';
            }
